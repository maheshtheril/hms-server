...
/* ───────────────── helpers ───────────────── */
async function tableExists(cx, schema, name) {
    const q = await cx.query(`SELECT 1 FROM information_schema.tables WHERE table_schema=$1 AND table_name=$2 LIMIT 1`, [schema, name]);
    return q.rowCount > 0;
}
async function columnExists(cx, schema, table, col) {
    const q = await cx.query(`SELECT 1 FROM information_schema.columns WHERE table_schema=$1 AND table_name=$2 AND column_name=$3 LIMIT 1`, [schema, table, col]);
    return q.rowCount > 0;
}
/** Collect role/permission “tokens” across common schemas and shapes. */
async function fetchUserRoleTokens(cx, tenantId, userId) {
    const tokens = new Set();
    // A) user_roles ↔ roles
    try {
        const { rows } = await cx.query(`SELECT COALESCE(NULLIF(TRIM(r.code), ''), NULLIF(TRIM(r.name), '')) AS t
       FROM public.user_roles ur
       JOIN public.roles r ON r.id = ur.role_id
       WHERE ur.tenant_id = $1 AND ur.user_id = $2`, [tenantId, userId]);
        rows.forEach((r) => r?.t && tokens.add(String(r.t)));
    }
    catch { }
    // B) role_users ↔ roles
    try {
       ...
       WHERE ru.tenant_id = $1 AND ru.user_id = $2`, [tenantId, userId]);
        rows.forEach((r) => r?.t && tokens.add(String(r.t)));
    }
    catch { }
    ...
    try {
        ...
        if (hasUsers) {
            const { rows } = await cx.query(`SELECT role, role_code, roles, role_codes, is_admin
         FROM public.users
         WHERE tenant_id=$1 AND id=$2 LIMIT 1`, [tenantId, userId]);
            if (rows[0]) {
                const r = rows[0];
                if (r.role)
                    tokens.add(String(r.role));
                if (r.role_code)
                    tokens.add(String(r.role_code));
                if (Array.isArray(r.roles))
                    r.roles.forEach((x) => x && tokens.add(String(x)));
                if (Array.isArray(r.role_codes))
                    r.role_codes.forEach((x) => x && tokens.add(String(x)));
                if (r.is_admin === true)
                    tokens.add("admin");
            }
        }
    ...
    try {
        ...
        if (hasUG && hasG) {
            const { rows } = await cx.query(`SELECT COALESCE(NULLIF(TRIM(g.code), ''), NULLIF(TRIM(g.name), '')) AS t
         FROM public.user_groups ug
         JOIN public.groups g ON g.id = ug.group_id
         WHERE ug.tenant_id = $1 AND ug.user_id = $2`, [tenantId, userId]);
            rows.forEach((r) => r?.t && tokens.add(String(r.t)));
        }
    }
    catch { }
    ...
    try {
        ...
        const hasUG = await tableExists(cx, "public", "user_groups");
        if (hasGP && hasP && hasUG) {
            const { rows } = await cx.query(`SELECT COALESCE(NULLIF(TRIM(p.code), ''), NULLIF(TRIM(p.name), '')) AS t
         FROM public.user_groups ug
...
router.get("/kpis", requireSession, async (req, res, next) => {
    ...
    if (!userId)
        return res.status(400).json({ error: "user_id_missing_in_session" });
    ...
    try {
        ...
        const { rows: sanity } = await cx.query(`SELECT COUNT(*)::int AS c FROM ${table} l WHERE l.tenant_id = $1`, [tenantId]);
        ...
        if (chosenCount === 0) {
            if (table === "public.leads" && hasLead) {
                const { rows: s2 } = await cx.query(`SELECT COUNT(*)::int AS c FROM public.lead l WHERE l.tenant_id = $1`, [tenantId]);
                const c2 = s2?.[0]?.c ?? 0;
                if (c2 > 0) {
                    table = "public.lead";
                    bare = "lead";
                ...
                const { rows: s2 } = await cx.query(`SELECT COUNT(*)::int AS c FROM public.leads l WHERE l.tenant_id = $1`, [tenantId]);
                const c2 = s2?.[0]?.c ?? 0;
                if (c2 > 0) {
                    table = "public.leads";
                    bare = "leads";
    ...
    `;
        ...
        console.log(`[kpis] table=${table} tenantRows=${chosenCount} scope=${mine ? "mine" : "all"} isAdmin=${isAdmin} tokens=${JSON.stringify(tokens)} open=${row.open_leads} today=${row.todays_followups}`);
        return res.json({
