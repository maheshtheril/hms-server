...
router.get("/", async (req, res, next) => {
    ...
    `;
        const cat = await db_1.pool.query(sql, params);
        // Count only canonical
        const catCount = await db_1.pool.query(`SELECT count(*)::int AS cnt FROM permission ${where}`, params);
        // Legacy discovery from role.permissions[] (for visibility during migration)
        const legacy = await db_1.pool.query(`
      WITH all_codes AS (
...
router.post("/", async (req, res, next) => {
    try {
        ...
        await db_1.pool.query(`INSERT INTO permission (code, name, description, category, is_deprecated)
       VALUES ($1,$2,$3,$4,$5)`, [code, name, description, category, is_deprecated]);
        const r = await db_1.pool.query(`SELECT code, name, description, category, is_deprecated
       FROM permission WHERE code=$1`, [code]);
...
router.put("/:code", async (req, res, next) => {
    try {
        ...
        if (is_deprecated !== undefined) {
            sets.push(`is_deprecated=$${i++}`);
            vals.push(is_deprecated);
        }
        if (!sets.length)
            return res.status(400).json({ error: "No fields to update" });
        vals.push(code);
        const r = await db_1.pool.query(`UPDATE permission SET ${sets.join(", ")}, created_at = created_at
       WHERE code=$${i}
       RETURNING code, name, description, category, is_deprecated`, vals);
        if (r.rowCount === 0)
