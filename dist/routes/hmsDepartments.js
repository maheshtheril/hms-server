...
function normalizeResult(resLike) {
    if (!resLike)
        return { rows: [], rowCount: 0, raw: resLike };
    if (Array.isArray(resLike))
        return { rows: resLike, rowCount: resLike.length, raw: resLike };
...
function mapPgErrorToResponse(err, res, defaultPayload) {
...
async function parentBelongsToTenantCompany(parentId, tenantId, companyId) {
    const pr = await (0, db_1.q)(`SELECT 1 FROM hms_departments WHERE id = $1 AND tenant_id = $2 AND company_id = $3 AND deleted_at IS NULL LIMIT 1`, [parentId, tenantId, companyId]);
    return normalizeResult(pr).rowCount > 0;
}
async function isParentCycle(candidateParentId, childId, tenantId, companyId) {
    let current = candidateParentId;
    for (let i = 0; i < PARENT_DEPTH_LIMIT; i++) {
        if (!isValidUUID(current))
            break;
        if (String(current).trim() === String(childId).trim())
            return true;
        const r = await (0, db_1.q)(`SELECT parent_id FROM hms_departments WHERE id = $1 AND tenant_id = $2 AND company_id = $3 LIMIT 1`, [current, tenantId, companyId]);
...
router.get("/:id", requireSession_1.default, async (req, res) => {
    try {
      ...
      LIMIT 1
    `;
        const r = await (0, db_1.q)(sql, [tenantId, id]);
        const nr = normalizeResult(r);
        if (nr.rowCount === 0)
 ...
 */
router.post("/", requireSession_1.default, requireWrite, async (req, res) => {
    try {
        const ss = req.session;
        if (!ss || !ss.tenant_id)
            return res.status(401).json({ error: "unauthenticated" });
        ...
        const body = req.body || {};
        const { name: rawName, company_id: rawCompanyId, code: rawCode = null, description: rawDescription = null, parent_id: rawParentId = null, is_active: rawIsActive = true, } = body;
        ...
        if (parent_id !== null && parent_id !== undefined) {
            if (!isValidUUID(parent_id))
                return res.status(400).json({ error: "invalid_parent_uuid" });
        ...
        if (code !== null && typeof code === "string" && code.length > 64)
            return res.status(400).json({ error: "invalid_code_length" });
    ...
    `;
        const r = await (0, db_1.q)(insertSql, [tenantId, company_id, name, code, description, parent_id, is_active, actor]);
        const nr = normalizeResult(r);
        ...
        if (err && err.code) {
            console.error("PG CODE:", err.code, "DETAIL:", err.detail, "CONSTRAINT:", err.constraint);
            if (err.code === "23505") {
                return res.status(409).json({ error: "department_conflict" });
            }
        }
 ...
 */
router.put("/:id", requireSession_1.default, requireWrite, async (req, res) => {
    try {
        const ss = req.session;
        if (!ss || !ss.tenant_id)
            return res.status(401).json({ error: "unauthenticated" });
        ...
        const actor = ss.user_id ?? null;
        const id = String(req.params.id || "").trim();
        const { name: rawName, company_id: rawCompanyId, code: rawCode = null, description: rawDescription = null, parent_id: rawParentId = null, is_active: rawIsActive = true, } = req.body || {};
        if (!isValidUUID(id))
            return res.status(400).json({ error: "invalid_id" });
        // Normalize + trim, with defensive handling for empty strings
        const name = typeof rawName === "string" ? rawName.trim() : rawName;
        ...
        if (!company_id || !isValidUUID(company_id))
            return res.status(400).json({ error: "invalid_company_id" });
        const existing = await (0, db_1.q)(`SELECT company_id FROM hms_departments WHERE tenant_id = $1 AND id = $2 LIMIT 1`, [
            tenantId,
            id,
        ]);
      ...
      UPDATE hms_departments
       ...
       RETURNING id, tenant_id, company_id, name, code, description, parent_id, is_active, created_by, updated_by, created_at, updated_at, deleted_at
    `;
        const r = await (0, db_1.q)(updateSql, [name, code, description, parent_id, is_active, actor, tenantId, id, company_id]);
        const nr = normalizeResult(r);
        if (nr.rowCount === 0)
            return res.status(404).json({ error: "not_found" });
 ...
 */
router.patch("/:id", requireSession_1.default, requireWrite, async (req, res) => {
    try {
        const ss = req.session;
        if (!ss || !ss.tenant_id)
            return res.status(401).json({ error: "unauthenticated" });
        ...
        if (!isValidUUID(id))
            return res.status(400).json({ error: "invalid_id" });
        const get = await (0, db_1.q)(`SELECT * FROM hms_departments WHERE tenant_id = $1 AND id = $2 LIMIT 1`, [tenantId, id]);
 ...
 */
router.delete("/:id", requireSession_1.default, requireWrite, async (req, res) => {
    try {
        const ss = req.session;
        if (!ss || !ss.tenant_id)
            return res.status(401).json({ error: "unauthenticated" });
        ...
        if (!isValidUUID(id))
            return res.status(400).json({ error: "invalid_id" });
        const r = await (0, db_1.q)(`UPDATE hms_departments
          SET is_active = false, deleted_at = now(), updated_by = $1, updated_at = now()
        WHERE tenant_id = $2 AND id = $3
      RETURNING id`, [actor, tenantId, id]);
        const nr = normalizeResult(r);
        if (nr.rowCount === 0)
            return res.status(404).json({ error: "not_found" });
