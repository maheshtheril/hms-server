...
router.get("/", async (req, res, next) => {
    ...
    try {
        ...
        await setTenant(db_1.pool, tenantId); // harmless if you don’t use it
        const params = [];
        let where = "WHERE 1=1";
        ...
        // Count
        const { rows: countRows } = await db_1.pool.query(`SELECT COUNT(*)::int AS total FROM public.audit_log ${where}`, params);
        const total = countRows[0]?.total ?? 0;
        // Page
        params.push(pageSize, offset);
        // Select, aliasing operation → action for the frontend
        const { rows: items } = await db_1.pool.query(`SELECT
         id,
         created_at,
         operation AS action,
         table_name,
