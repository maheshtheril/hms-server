...
const db_1 = require("../db"); // your pg helper: q(sql, params)
...
router.get("/", requireSession_1.default, async (req, res) => {
    try {
        ...
        const params = [tenant_id, company_id];
    ...
    `;
        const invoices = await (0, db_1.q)(sql, params);
...
router.post("/", requireSession_1.default, async (req, res) => {
    ...
    try {
        ...
        const result = await (0, db_1.q)(`SELECT * FROM public.hms_create_invoice($1::uuid, $2::uuid, $3::uuid, $4::uuid, $5::jsonb, $6::uuid)`, [tenant_id, company_id, patient_id, encounter_id, JSON.stringify(line_items), created_by]);
        ...
        if (due_at || currency) {
            const updates = [];
            const params = [tenant_id, company_id, invoice_id];
            let idx = params.length;
            if (due_at) {
                idx += 1;
            ...
            if (updates.length > 0) {
                await (0, db_1.q)(`UPDATE public.hms_invoice SET ${updates.join(", ")}, updated_at = now() 
           WHERE tenant_id = $1 AND company_id = $2 AND id = $3`, params);
            }
        }
        // Return fresh invoice row
        const invoiceRow = await (0, db_1.q)(`SELECT * FROM public.hms_invoice WHERE tenant_id = $1 AND company_id = $2 AND id = $3`, [tenant_id, company_id, invoice_id]);
...
router.get("/:id", requireSession_1.default, async (req, res) => {
    try {
        ...
        const invoice = await (0, db_1.q)(`SELECT i.*, COALESCE(jsonb_agg(l.*) FILTER (WHERE l.id IS NOT NULL), '[]') AS normalized_lines
       ...
       GROUP BY i.id`, [tenant_id, company_id, id]);
...
router.patch("/:id", requireSession_1.default, async (req, res) => {
    try {
        ...
        const cur = await (0, db_1.q)(`SELECT locked FROM public.hms_invoice WHERE tenant_id = $1 AND company_id = $2 AND id = $3`, [tenant_id, company_id, id]);
        if (cur.rowCount === 0) {
            return res.status(404).json({ success: false, error: "Invoice not found" });
        }
        ...
        const updates = [];
        const params = [tenant_id, company_id, id];
        let idx = params.length;
        if (due_at !== undefined) {
            idx += 1;
            updates.push(`due_at = $${idx}`);
      ...
      RETURNING *
    `;
        const updated = await (0, db_1.q)(sql, params);
...
router.delete("/:id", requireSession_1.default, async (req, res) => {
    try {
        ...
        const cur = await (0, db_1.q)(`SELECT locked, status, total_paid, total FROM public.hms_invoice WHERE tenant_id = $1 AND company_id = $2 AND id = $3`, [tenant_id, company_id, id]);
        if (cur.rowCount === 0)
            return res.status(404).json({ success: false, error: "Invoice not found" });
        ...
        await (0, db_1.q)(`DELETE FROM public.hms_invoice WHERE tenant_id = $1 AND company_id = $2 AND id = $3`, [
            tenant_id,
            company_id,
            id,
        ]);
