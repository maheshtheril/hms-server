...
router.get("/users", async (req, res, next) => {
    try {
        ...
        if (sort === "email")
            sortExpr = "LOWER(email)";
        else if (sort === "status")
            sortExpr = "(CASE WHEN is_active THEN 1 ELSE 0 END)";
        // WHERE
        ...
        // COUNT
        const { rows: crows } = await (0, db_1.q)(`SELECT COUNT(*)::int AS total
         FROM app_user
        WHERE ${where.join(" AND ")}`, params);
        const total = crows[0]?.total ?? 0;
        // PAGE
        params.push(pageSize, offset);
        const { rows: items } = await (0, db_1.q)(`SELECT
          id,
          email,
          name,
