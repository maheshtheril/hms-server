...
exports.computeAndStoreInsights = computeAndStoreInsights;
const db_1 = __importDefault(require("../db"));
/* ----------------------------- Heuristics ---------------------------- */
function heuristicRiskScore(patient) {
    let score = 10;
    if (!patient.dob)
        score += 20;
    if (!patient.gender)
        score += 15;
    if (!patient.contact || Object.keys(patient.contact || {}).length === 0)
        score += 15;
    if (patient.identifiers && Object.keys(patient.identifiers).length >= 2)
        score -= 10;
    if (patient.dob) {
...
function heuristicGender(patient) {
    try {
        const ids = patient.identifiers || {};
        if (ids?.national_id?.gender)
            return ids.national_id.gender;
        const fn = (patient.first_name || "").toLowerCase();
        if (fn.endsWith("a") && fn.length > 2)
            return "female";
    }
    catch { }
    return null;
}
function detectAnomalies(patient) {
    const anomalies = [];
...
async function callExternalLLMForSummary(patient) {
    if (!EXTERNAL_ALLOWED)
        return null;
    ...
    if (safePatient.notes)
        safePatient.notes = "[redacted]";
    const prompt = [
    ...
    try {
        ...
        if (err?.name === "AbortError")
            console.warn("LLM call timed out");
        else
...
/* ----------------------------- Main Function -------------------------- */
async function computeAndStoreInsights(patient) {
    ...
    if (llm) {
        if (typeof llm.risk === "number")
            risk = Math.round(Math.max(0, Math.min(100, llm.risk)));
        if (llm.gender)
            gender_suggestion = llm.gender;
        timeline_summary = llm.summary ?? null;
    }
    else {
        const parts = [];
        if (patient.dob)
            parts.push(`Born ${new Date(patient.dob).toLocaleDateString()}`);
        if (patient.patient_number)
            parts.push(`Patient #: ${patient.patient_number}`);
    ...
    `;
        ...
        const r = await db_1.default.query(upsertSql, params);
        const row = r.rows[0];
        return {
            id: row.id,
            patient_id,
