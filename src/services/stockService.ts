...
type ListOpts = {
  ...
  to?: string | null;   // ISO
  q?: string | null;    // search on sku/name/batch_no/reference
  limit?: number;
  offset?: number;
};

export async function listStockLedger(opts: ListOpts) {
  const params: any[] = [];
  let where = ` WHERE sl.tenant_id = $1 AND sl.company_id = $2 `;
  params.push(opts.tenantId);
  params.push(opts.companyId);

  if (opts.product_id) { params.push(opts.product_id); where += ` AND sl.product_id = $${params.length} `; }
  if (opts.batch_id) { params.push(opts.batch_id); where += ` AND sl.batch_id = $${params.length} `; }
  if (opts.movement_type) { params.push(opts.movement_type); where += ` AND sl.movement_type = $${params.length} `; }
  if (opts.reference) { params.push(opts.reference); where += ` AND sl.reference = $${params.length} `; }
  if (opts.from) { params.push(opts.from); where += ` AND sl.created_at >= $${params.length} `; }
  if (opts.to) { params.push(opts.to); where += ` AND sl.created_at <= $${params.length} `; }
...
/* Aggregated totals per product (sum of change_qty) */
type SummaryOpts = {
  tenantId: string;
  companyId: string;
  q?: string | null;
...
export async function summarizeStockByProduct(opts: SummaryOpts) {
  const params: any[] = [];
  let where = ` WHERE sl.tenant_id = $1 AND sl.company_id = $2 `;
  params.push(opts.tenantId);
  params.push(opts.companyId);
  ...
  `;
  const r = await q(sql, params);
  return r.rows.map((row: any) => ({ ...row, qty: Number(row.qty) }));
}

/* CSV export for ledger rows (simple, server-side generated) */
export async function exportLedgerCsv(opts: ListOpts) {
