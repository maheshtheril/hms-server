...
export default async function idempotency(req: Request & { session?: any }, res: Response, next: NextFunction) {
  ...
  try {
    ...
    const existing = await q(`SELECT response_status, response_body, processed_at FROM public.hms_idempotency_keys WHERE tenant_id = $1 AND key_text = $2`, [req.session.tenantId, key]);
    ...
    await q(`INSERT INTO public.hms_idempotency_keys (tenant_id, key_text, created_by, request_method, request_path, request_body) VALUES ($1,$2,$3,$4,$5,$6)`,
      [req.session.tenantId, key, req.session.userId || null, req.method, req.path, req.body ? JSON.stringify(req.body) : null]);
 ...
 */
export async function saveIdempotencyResponse(req: Request & { session?: any }, res: Response, status: number, body: any) {
  const key = res.locals.__idempotency_key;
  if (!key || !req.session) return;
  try {
    await q(
      `UPDATE public.hms_idempotency_keys SET response_status = $1, response_body = $2, processed_at = now() WHERE tenant_id = $3 AND key_text = $4`,
      [status, JSON.stringify(body), req.session.tenantId, key]
    );
  } catch (e) {
