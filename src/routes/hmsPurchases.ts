...
import { withTenantClient, TenantContext } from "../db/withTenantClient";
...
router.get("/", async (req: Request, res: Response) => {
  try {
    ...
    const rows = await withTenantClient(ctx, async (client: PoolClient) => {
      const { rows } = await client.query(
        `SELECT p.*, s.name AS supplier_name
         ...
         LIMIT $3 OFFSET $4`,
        [ctx.tenant_id, ctx.company_id, limit, offset]
      );
      return rows;
    });
    ...
    try {
      const result = await withTenantClient(ctx, async (client) => {
        ...
        const poIns = await client.query(
          `INSERT INTO public.hms_purchase_order
           (tenant_id, company_id, name, supplier_id, order_date, expected_date, currency, notes, created_by)
           VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
           RETURNING *`,
          [ctx.tenant_id, ctx.company_id, name, supplier_id, order_date || new Date(), expected_date || null, currency || "USD", notes || null, ctx.user_id || null]
        );
        ...
        for (let i = 0; i < lines.length; i++) {
          const ln = lines[i];
          if (!ln.product_id || !ln.qty) throw { status: 400, message: "Invalid line: missing product_id or qty" };
          ...
          const inserted = await client.query(
            `INSERT INTO public.hms_purchase_order_line
        ...
        // update totals
        await client.query(
          `UPDATE public.hms_purchase_order
           SET subtotal = $1, total_tax = $2, total_amount = $3, updated_at = now()
           WHERE id = $4`,
          [subtotal, totalTax, subtotal + totalTax, po.id]
        );
...
router.get("/:id", param("id").isUUID(), async (req: Request, res: Response) => {
  ...
  try {
    ...
    const { purchase_order, lines } = await withTenantClient(ctx, async (client) => {
      const poRes = await client.query(
        `SELECT p.*, s.name as supplier_name FROM public.hms_purchase_order p
         LEFT JOIN public.hms_supplier s ON s.id = p.supplier_id
         WHERE p.id = $1 AND p.tenant_id = $2 AND p.company_id = $3`,
        [id, ctx.tenant_id, ctx.company_id]
      );
...
/* -------------------- Update PO (patch) -------------------- */
router.put("/:id", param("id").isUUID(), async (req: Request, res: Response) => {
  try {
    const ctx = getCtx(req);
    const id = req.params.id;
    const payload = req.body;

    const updated = await withTenantClient(ctx, async (client) => {
      ...
      if (!poRes.rows.length) throw { status: 404, message: "PO not found" };
      ...
      const params: any[] = [];
      let idx = 1;
      if (payload.expected_date !== undefined) { up.push(`expected_date = $${idx++}`); params.push(payload.expected_date); }
      if (payload.notes !== undefined) { up.push(`notes = $${idx++}`); params.push(payload.notes); }
      if (payload.name) { up.push(`name = $${idx++}`); params.push(payload.name); }
      if (payload.supplier_id) { up.push(`supplier_id = $${idx++}`); params.push(payload.supplier_id); }

      if (up.length === 0) throw { status: 400, message: "Nothing to update" };
...
/* -------------------- Change status -------------------- */
router.put("/:id/status", param("id").isUUID(), body("status").isString(), async (req: Request, res: Response) => {
  const v = validationResult(req);
  if (!v.isEmpty()) return res.status(400).json({ errors: v.array() });
...
router.delete("/:id", param("id").isUUID(), async (req: Request, res: Response) => {
  try {
    ...
    await withTenantClient(ctx, async (client) => {
      ...
      if (!poRes.rows.length) throw { status: 404, message: "PO not found" };
      if (poRes.rows[0].status !== "draft") throw { status: 400, message: "Only draft POs can be deleted" };
