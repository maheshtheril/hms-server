...
function getCtx(req: Request): TenantContext {
  if (!req.session || !req.session.tenant_id || !req.session.active_company_id) throw new Error("Missing session context");
 ...
 */
router.post("/", body("lines").isArray({ min: 1 }), async (req: Request, res: Response) => {
  ...
  try {
    const created = await withTenantClient(ctx, async (client: PoolClient) => {
      const r = await client.query(
        `INSERT INTO public.hms_purchase_receipt
         (tenant_id, company_id, purchase_order_id, name, receipt_date, notes, created_by)
         VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`,
        [ctx.tenant_id, ctx.company_id, purchase_order_id, name, receipt_date || new Date(), notes, ctx.user_id || null]
      );

      const receipt = r.rows[0];

      for (const ln of lines) {
        if (!ln.product_id || !ln.qty) throw { status: 400, message: "Invalid receipt line" };

        await client.query(
          `INSERT INTO public.hms_purchase_receipt_line
      ...
      if (purchase_order_id) {
        ...
        const poQtyRes = await client.query(
          `SELECT COALESCE(SUM(qty),0) AS po_qty FROM public.hms_purchase_order_line WHERE purchase_order_id = $1`,
          [purchase_order_id]
        );
        const poQty = Number(poQtyRes.rows[0].po_qty || 0);

        const recRes = await client.query(
          `SELECT COALESCE(SUM(prl.qty),0) AS received
        ...
        if (poQty > 0 && received >= poQty) newStatus = "received";
