 ...
 * Production-ready professions router with soft-delete & restore
 */
...
router.param("id", (req, res, next, id) => {
  ...
  if (!valid) {
    if (DEBUG) console.warn("[lead_profession] invalid id:", id);
...
router.get("/", sessionLoader.loadSessionOptional, async (req, res, next) => {
  try {
    ...
    const sql = `
      SELECT id, tenant_id, name, category, description, is_active, created_by, created_at,
...
router.post("/", sessionLoader.requireSession, async (req, res, next) => {
  try {
    ...
    const insert = `
      INSERT INTO public.lead_profession
        (tenant_id, name, category, description, is_active, created_by, created_at, updated_at)
      VALUES ($1,$2,$3,$4,$5,$6,now(),now())
      RETURNING id, tenant_id, name, category, description, is_active, created_by, created_at, updated_at
    `;
    try {
      const { rows } = await db.query(insert, [tenant_id, name, category ?? null, description ?? null, is_active, created_by]);
...
router.get("/:id", sessionLoader.loadSessionOptional, async (req, res, next) => {
  try {
    ...
    const { rows } = await db.query(
      `SELECT id, tenant_id, name, category, description, is_active, created_by, created_at,
              COALESCE(updated_at, created_at) as updated_at, deleted_at, deleted_by
       FROM public.lead_profession WHERE id=$1 LIMIT 1`,
      [id]
    );
...
router.put("/:id", sessionLoader.requireSession, async (req, res, next) => {
  try {
    ...
    const existing = await db.query("SELECT id, tenant_id FROM public.lead_profession WHERE id=$1 LIMIT 1", [id]);
    ...
    const { rows } = await db.query(
      `UPDATE public.lead_profession
         SET name=$1, category=$2, description=$3, is_active = COALESCE($4, is_active), updated_at = now()
       WHERE id=$5
       RETURNING id, tenant_id, name, category, description, is_active, created_by, created_at, updated_at, deleted_at, deleted_by`,
      [name, category ?? null, description ?? null, is_active, id]
    );
    if (!rows.length) return res.status(404).json({ error: "not_found" });
    return res.json({ data: rows[0] });
  } catch (e: any) {
    if (e?.code === "23505") return res.status(409).json({ error: "duplicate_name" });
    DEBUG && console.error("[lead_profession] PUT error:", e);
    next(e);
  }
});
