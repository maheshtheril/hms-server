...
router.get("/:leadId/custom-fields", async (req: any, res) => {
  try {
    ...
    `;
    const { rows } = await pool.query(q, [tenantId, leadId]);
    res.json(rows);
  } catch (err: any) {
 ...
 *  { value_text } | { value_number } | { value_boolean } | { value_json }
 */
router.put("/:leadId/custom-fields/:definitionId", async (req: any, res) => {
  try {
    const tenantId = getTenantId(req);
    ...
    const cols: string[] = [];
    const vals: any[] = [tenantId, leadId, definitionId, userId || null];
    ...
    const updateAssignments = cols.map(
      (c, i) => `${c} = EXCLUDED.${c}`
    );
