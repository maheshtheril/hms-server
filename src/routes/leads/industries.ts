...
router.param("id", (req, res, next, id) => {
  ...
  if (!valid) {
    if (DEBUG) console.warn("[lead_industry] invalid id:", id);
...
router.post("/", sessionLoader.requireSession, async (req, res, next) => {
  try {
    ...
    const insert = `
      INSERT INTO public.lead_industry
...
router.get("/:id", sessionLoader.loadSessionOptional, async (req, res, next) => {
  try {
    ...
    const { rows } = await db.query(
      `SELECT id, tenant_id, name, description, is_active, created_by, created_at,
              COALESCE(updated_at, created_at) as updated_at, deleted_at, deleted_by
       FROM public.lead_industry WHERE id=$1 LIMIT 1`,
      [id]
    );
...
router.put("/:id", sessionLoader.requireSession, async (req, res, next) => {
  try {
    ...
    const existing = await db.query("SELECT id, tenant_id FROM public.lead_industry WHERE id=$1 LIMIT 1", [id]);
    ...
    if (name !== null && name.length === 0) return res.status(400).json({ error: "name_required" });
    ...
    const { rows } = await db.query(
      ...
      [name ?? null, description ?? null, is_active, id]
    );
    if (!rows.length) return res.status(404).json({ error: "not_found" });
    return res.json({ data: rows[0] });
  } catch (e: any) {
    if (e?.code === "23505") return res.status(409).json({ error: "duplicate_name" });
    DEBUG && console.error("[lead_industry] PUT error:", e);
    next(e);
  }
});
