...
function assertTenant(req: any): string {
  const raw = String(req.session?.tenant_id || req.headers["x-tenant-id"] || "").trim();
  if (!raw) throw Object.assign(new Error("tenant_id_required"), { status: 400 });
  if (!isUuid(raw)) throw Object.assign(new Error("invalid tenant_id"), { status: 400 });
  return raw;
}
...
router.post("/", async (req: any, res: any, next: any) => {
  ...
  try {
    ...
    const dup = await cx.query(
      `select 1 from public.app_user where tenant_id = ${TENANT_UUID_SQL} and lower(email)=lower($1) limit 1`,
      [email]
    );
    if (dup.rowCount) {
    ...
    if (company_id) {
      ...
      const ok = await cx.query(
        `select 1 from public.company where id = $1 and tenant_id = ${TENANT_UUID_SQL} limit 1`,
        [company_id]
      );
      if (!ok.rowCount) {
        await cx.query("ROLLBACK");
      ...
      const companies = await cx.query(
        `select id from public.company where tenant_id = ${TENANT_UUID_SQL} order by created_at asc nulls last`
      );
    ...
    const ins = await cx.query(
      `insert into public.app_user (tenant_id, email, name, password, is_active, company_id)
       values (${TENANT_UUID_SQL}, $1, $2, $3, true, $4)
       returning id, email, name, company_id, is_active, created_at`,
      [email, name, hash, finalCompanyId]
    );
    const userId: string = ins.rows[0].id;

    // Assign roles by role.key (tenant-scoped or global)
    if (Array.isArray(roles) && roles.length) {
      const ids = await cx.query(
        `select id from public.role
          where key = any($1::text[])
            and (tenant_id = ${TENANT_UUID_SQL} or tenant_id is null)`,
        [roles]
      );
      ...
      await cx.query(
        `insert into public.user_role (user_id, role_id) values ${values} on conflict do nothing`,
        [userId, ...ids.rows.map(r => r.id)]
      );
    }

    // Ensure user_companies membership + default flag (PLURAL table)
    await cx.query(
      `insert into public.user_companies (tenant_id, user_id, company_id, is_default)
...
router.get("/", async (req: any, res: any, next: any) => {
  ...
  try {
    ...
    const tenantId: string = String(req.session?.tenant_id || req.headers["x-tenant-id"] || "").trim();
    if (!tenantId) return res.status(401).json({ error: "unauthenticated" });

    // Robust boolean coercion in case flags arrive as strings
    const asBool = (v: any) => v === true || v === 1 || String(v).toLowerCase?.() === "true" || v === "t";
...
router.get("/:id", async (req: any, res: any, next: any) => {
  ...
  try {
    ...
    const { id } = req.params;
    if (!isUuid(id)) return res.status(400).json({ error: "bad_request", message: "id must be a UUID" });
    ...
    const { rows } = await cx.query(
      `SELECT
       ...
       GROUP BY u.id
       LIMIT 1`,
      [id, !!req.session?.is_platform_admin]
    );
...
router.patch("/:id", async (req: any, res: any, next: any) => {
  ...
  try {
    ...
    // attach roles[]
    const r = rows[0];
    const rolesRes = await cx.query(
      `SELECT COALESCE(ARRAY_REMOVE(ARRAY_AGG(DISTINCT r.key), NULL), '{}')::text[] AS roles
       FROM public.user_role ur
       JOIN public.role r ON r.id = ur.role_id
       WHERE ur.user_id = $1`,
      [id]
...
/* ───────── RESEND INVITE (stub) ───────── */
router.post("/:id/resend-invite", async (req: any, res: any, next: any) => {
  const cx = await pool.connect();
  try {
    await setTenantOn(cx, req);
    const { id } = req.params;
