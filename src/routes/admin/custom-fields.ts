...
async function hydrateSession(req: any) {
  ...
  if (sess) {
    ...
    pool.query(`update session set last_seen_at = now() where sid = $1`, [sid]).catch(() => {});
  }
}

async function setTenant(req: any) {
  const tenantId = req.session?.tenant_id || (req.headers["x-tenant-id"] as string) || null;
  if (tenantId) {
    await pool.query(`select set_config('app.tenant_id', $1, true)`, [String(tenantId)]);
  }
}
function getTenantId(req: any): string | null {
  return (req.session?.tenant_id || (req.headers["x-tenant-id"] as string) || null) ?? null;
}
...
router.get("/", async (req: any, res, next) => {
  try {
    ...
    if (!tenantId) return res.status(401).json({ error: "unauthorized", message: "Missing tenant/session" });
    ...
    const wh: string[] = [`tenant_id = $1`, `entity = $2`];
    const vals: any[] = [tenantId, entity];
...
router.patch("/:id", async (req: any, res, next) => {
  try {
    ...
    const exists = await pool.query(
      `select 1 from public.custom_field_definition where id=$1 and tenant_id=$2`,
      [id, tenantId]
    );
...
router.delete("/:id", async (req: any, res, next) => {
  try {
    ...
    const r = await pool.query(
      `delete from public.custom_field_definition where id=$1 and tenant_id=$2`,
      [id, tenantId]
    );
