...
const LeadCreateSchema = z.object({
  ...
  priority: z.coerce.number().min(1).max(5).default(3),
    custom_data: z.record(z.string(), z.any()).default({} as Record<string, any>),
  tags: z.array(z.string()).default([]),
  meta: z.record(z.string(), z.any()).default({} as Record<string, any>),
  metadata: z.record(z.string(), z.any()).default({} as Record<string, any>),
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      if (v.dry_run) {
        ...
        if (v.primary_phone && v.primary_phone.trim()) {
          const p = parsePhoneNumberFromString(v.primary_phone.trim(), "IN");
      ...
      try {
        ...
        const params = [
          ...
          v.stage_id ?? null,
          v.name,
          v.primary_email || null,
          v.primary_phone || null,
          e164,
          v.source_id ?? null,
          v.status,
          v.estimated_value,
          ...
          v.meta,
          v.metadata,
          v.profession_id ?? null,
          v.profession_name ?? null,
          v.industry_id ?? null,
          who,
        ];
        ...
        if (v.task?.enabled && v.task?.title?.trim()) {
          await db.query(
            ...
            [
              ...
              leadId,
              v.task.title.trim(),
              v.task.due_date ?? null,
              v.task.status ?? "open",
              v.task.assigned_to ?? null,
              who,
              v.company_id ?? null,
            ]
          );
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const followups = (await db.query(
        `SELECT * FROM public.lead_followups WHERE lead_id=$1 ORDER BY created_at DESC`,
        [id]
      )).rows;
      const notes = (await db.query(
        `SELECT * FROM public.lead_note WHERE lead_id=$1 ORDER BY created_at DESC`,
        [id]
      )).rows;
      const tasks = (await db.query(
        `SELECT * FROM public.lead_task WHERE lead_id=$1 ORDER BY created_at DESC`,
        [id]
      )).rows;
...
leadsNewRouter.put(
  "/new/leads/:id",
  async (req: Request, res: Response, next: NextFunction) => {
    const who = getUserId(req as any);
...
leadsNewRouter.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (pipeline_id) {
        filters.push(`pipeline_id=$${i++}`);
        params.push(pipeline_id);
      }
      if (stage_id) {
        filters.push(`stage_id=$${i++}`);
        params.push(stage_id);
      }
      if (owner_id) {
        filters.push(`owner_id=$${i++}`);
...
leadsNewRouter.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    const leadId = req.params.id;
    const { to_stage_id, tenant_id } = req.body as any;
    const who = getUserId(req as any);
    try {
      const old = (await db.query(
        `SELECT stage_id, stage, pipeline_id FROM public.lead WHERE id=$1`,
        [leadId]
      )).rows[0];
...
leadsNewRouter.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { source_id, target_id, tenant_id } = req.body as any;
