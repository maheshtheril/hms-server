...
function assertStr(o: any, k: string, required = false) {
  if (!(k in o)) { if (required) throw new Error(`${k} required`); else return; }
  if (o[k] != null && typeof o[k] !== "string") throw new Error(`${k} must be string`);
}
